    ...
    <div class="bom">
      ...
      <div class="bom-table">
        ...
        <ul v-for="(item , idx ) in tableBom" :key="idx">
          ...
          <li>{{item.IR}}</li>
          <li style="text-align: center;">{{parFloat(item.Size)}}</li>
          <li style="text-align: center;">{{item.underlying_balance!==0?parFloat(item.pnls):'NaN'}}</li>
          <li>
  ...
  methods: {
    ...
    setShou () {
      if (this.btnbid) {
        ...
        const number = new BigNumber(this.bidData.Size)
          .multipliedBy(Digits[this.Smile])
          .toString(10)
    ...
    getMyBFs (item) {
      if (item.total_supply === 0) {
        const c = '-' + new BigNumber(item.value_for_claim).dividedBy(Digits[this.Smile]).toString(10)
        ...
        getMyBF(this.MetaMaskAddress, item.lp_addr).then(res => {
          ...
          const a = new BigNumber(mybf).multipliedBy(item.underlying_balance).dividedBy(item.total_supply)
            .toString(10)
          const b = new BigNumber(a).minus(item.value_for_claim).dividedBy(Digits[this.Smile]).toString(10)
